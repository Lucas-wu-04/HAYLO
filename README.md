...
| Wordmark 字重 | Comfortaa 500 + 字距 .005em | **Comfortaa 400、零字距**（對齊官方 lockup）|
...
`icons/` 依品牌原稿 `Untitled_design.png` 量測後重畫成向量，不是縮圖：

- 版面：滿版正方形、直角、`#000000`
- 圓環：置中，外半徑 43.75%、線寬 4.88%、`#FFFFFF`
- 沒有圓點（Android 專案舊向量 `ic_launcher_foreground.xml` 有一顆，現行 mark 已移除）

驗證：`icon-512.png` 與原稿的中線墨水區間完全對齊（32–56、455–479），
整圖墨水覆蓋率 0.12659 vs 原稿 0.12679。

`icon-maskable-512.png` 是唯一調整過的 — 外半徑縮到 40%，因為 maskable 規範
只保證直徑 80% 的圓形安全區，43.75% 會被 launcher 的遮罩切到。

要改 logo：改 `icons/favicon.svg` 裡的 `rect` + `circle`，再跑 `python3 tools/build-icons.py`，
六個尺寸一次重出。
